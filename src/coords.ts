...
import { formatDms } from './geodesy.ts'
...
function axisOfSuffix(suffix: string | undefined): 'lat' | 'lon' | undefined {
  if (suffix === undefined) return undefined
  const upper = suffix.toUpperCase()
  if (upper === 'N' || upper === 'S') return 'lat'
  if (upper === 'E' || upper === 'W') return 'lon'
  return undefined
}

function signOfSuffix(suffix: string | undefined): number {
  const upper = suffix === undefined ? '' : suffix.toUpperCase()
  if (upper === 'S' || upper === 'W') return -1
  return 1
}
...
function checkRange(value: number, isLat: boolean): string | undefined {
...
export function parseToken(
  ...
  kindHint: CoordKind,
  suffixAxis?: 'lat' | 'lon',
): ParsedCoord | { error: string } {
  const token = raw.trim()
  let decimal = DECIMAL_RE.exec(token)
  let dms = null
  if (decimal === null) dms = DMS_RE.exec(token)
  ...
  if (dms !== null) {
    ...
    if (min >= 60) return error(`invalid minutes ${minPart} in "${token}" (must be < 60)`)
    let seconds = 0
    if (secPart !== undefined) {
      seconds = Number.parseFloat(secPart)
      if (seconds >= 60) return error(`invalid seconds ${secPart} in "${token}" (must be < 60)`)
    }
...
export function parseCoordInput(
  input: string,
  kind: CoordKind,
): CoordPair | ParsedCoord | { error: string } {
...
function parseOrderedPair(first: string, second: string, original: string): CoordPair | { error: string } {
  ...
  if ('error' in lon) return lon
  return { latitude: lat.value, longitude: lon.value }
}

/** Format both axes of a pair as DMS strings. */
export function formatPairDms(lat: number, lon: number): { latDms: string; lonDms: string } {
