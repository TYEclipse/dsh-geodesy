...
import { DEFAULT_RADIUS_KM, haversineKm, type Point } from './geodesy.ts'
...
function tangentToward(v: Vec3, t: Vec3): Vec3 | undefined {
  const d = dot(t, v)
  const raw = { x: t.x - d * v.x, y: t.y - d * v.y, z: t.z - d * v.z }
  const n = Math.sqrt(dot(raw, raw))
  if (n < 1e-9) return undefined // identical or antipodal neighbors
